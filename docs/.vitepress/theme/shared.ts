import type { AdditionalConfig, HeadConfig, LocaleSpecificConfig, PageData, SiteData } from 'vitepress'
...
export function isActive(
  ...
  if (hashMatch) {
    return (inBrowser ? globalThis.location.hash : '') === hashMatch[0]
  }
...
export function resolveSiteDataByRoute(
  ...
  const topLayer = {
    head: mergeHead(
      siteData.head ?? [],
      (localeConfig as LocaleSpecificConfig).head ?? [],
...
export function treatAsHtml(filename: string): boolean {
  if (KNOWN_EXTENSIONS.size === 0) {
    const extraExts
      ...
      = (typeof globalThis.process === 'object' && globalThis.process.env?.VITE_EXTRA_EXTENSIONS)
    ...
    for (const ext of (
      ...
      .split(',')) KNOWN_EXTENSIONS.add(ext)
  }
